...
export class AutocadastroComponent implements OnInit {
  ...
  campoComErro(campo: string, erro: string): boolean {
    const control = this.form.get(campo);

    return !!control?.touched && control.hasError(erro);
  }

  private quantidadeDigitos(quantidade: number) {
  ...
  private quantidadeDigitosEntre(minimo: number, maximo: number) {
    return (control: AbstractControl): ValidationErrors | null => {
      const valor = this.somenteDigitos(control.value);
  ...
  private montarPayload(): AutocadastroInput {
    const valor = this.form.controls.salario.value;
    const complemento = this.form.controls.endereco.controls.complemento.value;

    return {
      cpf: this.somenteDigitos(this.form.controls.cpf.value),
      nome: this.form.controls.nome.value!,
      email: this.form.controls.email.value!,
      telefone: this.somenteDigitos(this.form.controls.telefone.value),
      salario: valor !== null ? paraContrato(paraDecimal(valor.toString())) : '',
      endereco: {
        logradouro: this.form.controls.endereco.controls.logradouro.value!,
        numero: this.form.controls.endereco.controls.numero.value!,
        complemento: complemento?.trim() ? complemento.trim() : null,
        cep: this.somenteDigitos(this.form.controls.endereco.controls.cep.value),
        cidade: this.form.controls.endereco.controls.cidade.value!,
        uf: this.form.controls.endereco.controls.uf.value!,
      },
    };
  }
  ...
  async enviar(): Promise<void> {
    ...
    if (this.form.invalid) {
      ...
      this.mensagemErro.set('Verifique os campos destacados.');
      return;
    }
