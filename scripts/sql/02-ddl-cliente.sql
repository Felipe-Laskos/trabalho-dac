...
CREATE TABLE cliente.solicitacoes (
    cpf VARCHAR(11) PRIMARY KEY,
    nome VARCHAR(120) NOT NULL,
    email VARCHAR(120) NOT NULL UNIQUE,
    telefone VARCHAR(20) NOT NULL,
...
CREATE INDEX ix_solicitacoes_status ON cliente.solicitacoes (status);
CREATE INDEX ix_clientes_nome ON cliente.clientes (nome);

CREATE TABLE cliente.comandos_processados (
